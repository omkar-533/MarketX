/**
 * Adds how_to_video_url to app_indicators (idempotent).
...
async function probeColumn() {
  ...
  const { error } = await db.from('app_indicators').select('id, how_to_video_url').limit(1);
...
async function main() {
  ...
  if (before.ok) {
    console.log('Already present: how_to_video_url is live.');
    return;
  }
  ...
  if (accessToken) await viaManagementApi();
    ...
    // Last resort: try service role against Management API (usually rejected).
    if (serviceKey) {
      const ref = projectRef();
      const res = await fetch(`https://api.supabase.com/v1/projects/${ref}/database/query`, {
        method: 'POST',
  ...
  console.log('Verified: how_to_video_url is live.');
}
