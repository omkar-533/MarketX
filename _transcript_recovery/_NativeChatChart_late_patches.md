# NativeChatChart late StrReplace patches (apply order)
...
```tsx
            ...
            {wolfActive.length ? (
              <span className="mai-nc__legend-wolf">
                {wolfActive.map((w) => (
                  <span key={w.id} className="mai-nc__wolf-chip" title={w.description}>
                    {w.label}
                    {w.status === 'stub' ? <em>Soon</em> : null}
                  </span>
                ))}
              </span>
            ) : null}
