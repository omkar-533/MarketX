...
import type { ScreenerMarketRow } from '../services/screenerDataService';
import {
...
interface ReadyMadeScreenersProps {
  ...
  loading?: boolean;
  feedLabel?: string;
  feedMode?: 'live' | 'mixed' | 'offline' | string;
  onRefresh?: () => void;
}
...
export default function ReadyMadeScreeners({
  rows,
  loading,
  feedLabel,
  feedMode,
  onRefresh,
}: ReadyMadeScreenersProps) {
  const [query, setQuery] = useState('');

  const grouped = useMemo(() => {
    const all = runAllReadyMadeScreeners(rows, 5);
